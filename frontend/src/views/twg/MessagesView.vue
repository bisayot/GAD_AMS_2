<template>
  <main class="messages-main-content">
    <div class="messages-content-wrapper">
      ...
      <div class="info-note" style="margin: 0 0 2rem 0; padding: 1.5rem 2rem; background-color: #1e293b; border: 1px solid rgba(147, 51, 234, 0.3); border-left: 4px solid #9333ea; border-radius: 0.75rem; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3); display: flex; align-items: flex-start; gap: 1.25rem;">
        <div style="background: rgba(147, 51, 234, 0.2); padding: 0.5rem; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
          <span class="material-symbols-outlined" style="color: #c084fc; font-size: 1.5rem;">info</span>
        </div>
        <div>
            ...
            <div v-if="showCreateModal" class="create-message-panel">
              ...
              <div class="panel-body">
                ...
                <div class="form-group">
                  ...
                  <!-- User Selection -->
                  <div v-if="selectedRole" class="filter-group">
                ...
                <div v-for="message in messages" :key="message.id" class="message-card" @click="toggleMessageExpand(message)" style="cursor: pointer; position: relative;">
                  ...
                  <transition name="expand">
                    <div v-if="expandedMessageId === message.id" class="message-expanded-content" style="margin-top: 1rem; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 1rem;">
                      ...
                      <div v-if="message.document_id" class="message-attachment" style="margin-top: 1.5rem; padding: 0.75rem 1rem; background: rgba(147, 51, 234, 0.1); border-radius: 0.5rem; display: inline-flex; align-items: center; gap: 0.75rem; border: 1px solid rgba(185, 121, 204, 0.2);">
  ...
  if (user.value.id) {
    try {
      const [designsRes, reportsRes, archivesRes] = await Promise.all([
        ...
        api.get(`archives?user_id=${user.value.id}&role=${user.value.user_role || 'TWG'}`)
      ]);
...
const replyToMessage = async (message) => {
  ...
  replyAllUserDocuments.value = allUserDocuments.value;
};
...
const sendReply = async () => {
  ...
  try {
    const payload = {
      ...
      document_type: replyDocuments.value.length > 0 ? 'Mixed' : null,
...
const viewAttachedDesign = (docId) => {
  ...
  router.push({ name: 'college-ad-view', params: { id } });
};

const viewAttachedReport = (docId) => {
  if (!docId) return;
  const id = docId.replace('report_', '');
  router.push({ name: 'college-ar-view', params: { id } });
};
